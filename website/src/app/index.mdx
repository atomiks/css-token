...
<div
  style={css({
    ...
    color: token("red-10"),
    "&:p3": {
      backgroundColor: token("red-2-p3"),
      color: token("red-10-p3"),
    },
  })}
/>
```
...
Shadows presets are pre-defined shadows available as CSS variables:
...
<div
  style={{
    // 1-9
    fontWeight: token("tracking-5"),
    // tight, snug, normal, relaxed, loose
    fontWeight: token("tracking-loose"),
  }}
/>
```
...
<div
  style={{
    // 1-9
    fontWeight: token("leading-5"),
    // tight, snug, normal, relaxed, loose
    fontWeight: token("leading-loose"),
  }}
/>
```
...
Lush gradients in `oklch` color space (by default) for text.
